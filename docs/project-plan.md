...
1. 在桌面模式启动一个服务端和两个 Godot 客户端，目检权威位置与方向输入。
2. 为远端实体增加快照缓冲与插值，并显示快照年龄。
3. 为本地实体增加输入确认、预测、校正与未确认输入重放。
...
阶段 0、阶段 1 和阶段 2 已在 MSVC 下完成配置、构建、测试与本机 UDP 验证；阶段 2 已验证 `InputCommand -> 权威模拟 -> WorldSnapshot` 闭环。
